...
def list_conversations(status: str | None = None, priority: str | None = None, db: Session = Depends(get_db)):
    ...
    items = query.all()
    return {"count": len(items), "conversations": [{"conversation_id": c.id, "customer_id": c.customer_id, "channel": c.channel, "status": c.status, "priority": c.priority, "created_at": c.created_at} for c in items]}

@router.get("/conversations/{conversation_id}")
def conversation_detail(conversation_id: str, db: Session = Depends(get_db)):
    conversation = db.get(Conversation, conversation_id)
    if conversation is None: return {"detail": "Conversation not found"}
    ...
    return {"conversation_id": conversation.id, "customer_id": conversation.customer_id, "channel": conversation.channel, "status": conversation.status, "priority": conversation.priority, "messages": [{"id": m.id, "sender_type": m.sender_type, "content": m.content, "created_at": m.created_at} for m in messages], "tickets": [{"id": t.id, "status": t.status, "priority": t.priority, "summary": t.summary, "created_at": t.created_at} for t in tickets]}
