...
Lệnh python -m unittest discover -s app/tests -v chạy đạt 27 kiểm thử trên DB thử tách biệt. Các nhóm kiểm tra bao gồm mật khẩu/cookie, phiên hết hạn và tài khoản vô hiệu, quyền admin/agent và CSRF, giới hạn đăng nhập, dữ liệu gửi giả mạo, luồng tin nhắn/handoff, nhận tranh chấp bằng hai luồng, migration giữ dữ liệu, Inbox, nhận diện mã đơn, vector lưu bền, metadata, chống trùng, retry/xóa tài liệu, citation sai, lỗi provider, AI muộn sau tin khách mới và công thức chấm benchmark.
...
Lỗi chính gồm quote đổi dấu câu gây từ chối, hiểu câu trả lời phủ định thành thiếu bằng chứng, tự chọn sản phẩm khi câu hỏi mơ hồ và chọn một nguồn mâu thuẫn. Test còn lấy tỷ lệ cộng điểm làm tỷ lệ đổi tiền mặt, chấp nhận hai yêu cầu phải từ chối do injection. Một thử nghiệm prompt trên dev không tăng decision accuracy và gây thêm đáp án bịa nên đã bị loại trước khi chạy test. Giữ cấu hình ứng dụng và ngưỡng 0,35; không suy từ citation hợp lệ về chuỗi sang mọi mệnh đề đều có căn cứ. Công thức, kết quả và danh sách ca cần duyệt nằm trong docs/RAG_EVALUATION.md và evals/rag/.
...
Mốc hiện tại tạo nền tảng xác thực và chuyển giao có người phụ trách, giúp giữ nhất quán quyền trả lời và trạng thái AI. RAG local đã có mô hình thật và lưu bền. Đã có bộ 72 câu và baseline ban đầu. Công việc tiếp theo là duyệt nhãn, kiểm chứng từng mệnh đề với nguồn và sửa câu mơ hồ/mâu thuẫn/injection; sau đó hoàn thiện widget/realtime/SLA, Agentic RAG, kênh thứ hai, đánh giá và triển khai.
